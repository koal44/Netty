    ...
    public class AutoGrid : Grid
    {
        ...
        */
        ...
        private void ApplyChildLayout(UIElement child)
        {
            if (ChildMargin != null)
            {
                SetIfDefault(child, FrameworkElement.MarginProperty, ChildMargin.Value);
            }
            if (ChildHorizontalAlignment != null)
            {
                SetIfDefault(child, FrameworkElement.HorizontalAlignmentProperty, ChildHorizontalAlignment.Value);
            }
            if (ChildVerticalAlignment != null)
            {
                SetIfDefault(child, FrameworkElement.VerticalAlignmentProperty, ChildVerticalAlignment.Value);
            }
        }

        public static bool SetIfDefault<T>(DependencyObject o, DependencyProperty property, T value)
        {
            if (DependencyPropertyHelper.GetValueSource(o, property).BaseValueSource == BaseValueSource.Default)
            {
                o.SetValue(property, value);
